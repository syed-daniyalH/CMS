...
import { GenericDropdownObj, getCustomers } from '../../store/dropdowns'
...
interface Props {
  ...
  showCustomersOnly?: boolean | null
...
const CustomerSelector = ({
    ...
    if (isActive) {
      if (selected_value && `${object?.value}` !== `${selected_value}`) {
        if (objectList.length || forceStore) {
          ...
          axios.get(`/Dropdowns/GetActiveCustomers`, {
            params: {
              CustomerId: selected_value
            }
  ...
  useEffect(() => {
    ...
    if (isActive) {
      ...
      if (store?.customers?.length || store?.customers_success) {
            setObjectList(store?.customers??[]);
      } else {
        dispatch(
          getCustomers({})
        )
    ...
    <>
      <CustomAutocomplete
        ...
        renderInput={params => <CustomTextField {...params} noBorder={noBorder} fullWidth
                                                placeholder={t("Select Customer") as string}
                                                label={<TypoLabel name={'Customer'} />} {...(props ?? {})} />}
