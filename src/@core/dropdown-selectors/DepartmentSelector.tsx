...
import {GenericDropdownObj, getDepartments} from "../../store/dropdowns";
import {useDispatch, useSelector} from "react-redux";
import {AppDispatch, RootState} from "src/store";
import {useTranslation} from "react-i18next";
import {TextFieldProps} from "@mui/material/TextField";
import {AutocompleteChangeDetails, AutocompleteChangeReason, Box} from "@mui/material";
import {addNewDepartmentKey, departmentKey} from "../utils/translation-file";
...
const DepartmentSelector = ({selected_value, handleChange, clearable = true, canAdd = false, disabled = false, props}: Props) => {
    ...
    if (isActive) {
      ...
      if (selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Defaults/GetEmployeeDepartments`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if (isActive) {
      ...
      if (store.departments?.length || store.departments_success) {
        setObjectList(store.departments)
      } else {
        dispatch(
          getDepartments({})
        )
  ...
  return (
    <>
      <CustomAutocomplete
        ...
        renderInput={params => <CustomTextField {...params} fullWidth label={t(departmentKey)}  placeholder={t(departmentKey) as string} {...(props ?? {})} />}
...
export default React.memo(DepartmentSelector);
