...
import {GenericDropdownObj, getDepreciationMethods} from "../../store/dropdowns";
...
const DepreciationMethodSelector = ({selected_value, handleChange, clearable = true, disabled = false, important = false, props} : Props) => {
    ...
    if(isActive) {
      ...
      if(selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Defaults/GetFixedAssetsDepreciationMethods`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if(isActive) {
      ...
      if(store.depreciation_methods?.length || store.depreciation_methods_success) {
        setObjectList(store.depreciation_methods)
      } else {
        dispatch(
  ...
  }, [store.depreciation_methods, dispatch])
  ...
  return (
    <CustomAutocomplete
      ...
      renderInput={params => <CustomTextField {...params} fullWidth label={<TypoLabel name={t('Depreciation Method')} important={important??false} />} {...(props??{})} />}
    />
  )
}

export default React.memo(DepreciationMethodSelector);
