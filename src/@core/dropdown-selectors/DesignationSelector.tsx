...
import {GenericDropdownObj, getDesignations} from "../../store/dropdowns";
import {useDispatch, useSelector} from "react-redux";
import {AppDispatch, RootState} from "src/store";
import {useTranslation} from "react-i18next";
import {TextFieldProps} from "@mui/material/TextField";
import {AutocompleteChangeDetails, AutocompleteChangeReason, Box} from "@mui/material";
import {addNewDesignationKey, designationKey} from "../utils/translation-file";
...
const addNewDesignation: GenericDropdownObj = {
  value: -1,
  text: "Add New Designation",
  code: '-1'
}

const DesignationSelector = ({selected_value, handleChange, clearable = true, canAdd = false, disabled = false, props}: Props) => {
    ...
    if (isActive) {
      ...
      if (selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Defaults/GetEmployeeDesignations`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if (isActive) {
      ...
      if (store.designations?.length || store.designation_success) {
        setObjectList(store.designations)
      } else {
        dispatch(
          getDesignations({})
        )
  ...
  return (
    <>
      <CustomAutocomplete
        options={canAdd ? [addNewDesignation, ...objectList ?? []] : objectList ?? []}
        sx={{mb: 3}}
        ...
        renderInput={params => <CustomTextField {...params} fullWidth label={t(designationKey)} placeholder={t(designationKey) as string}  {...(props ?? {})} />}
