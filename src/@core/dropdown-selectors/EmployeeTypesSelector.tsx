...
import {GenericDropdownObj, getEmployeeTypes} from "../../store/dropdowns";
...
const EmployeeTypesSelector = ({selected_value, handleChange, clearable = true, readOnly = false, disabled = false, props}: Props) => {
    ...
    if (isActive) {
      ...
      if (selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Defaults/GetPayRollEmployeeTypes`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if (isActive) {
      ...
      if (store.emp_types?.length || store.emp_types_success) {
        setObjectList(store.emp_types)
      } else {
        dispatch(
          getEmployeeTypes({})
        )
...
export default React.memo(EmployeeTypesSelector);
