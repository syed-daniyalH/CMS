...
import { GenericDropdownObj, getProjectTypes } from '../../store/dropdowns'
...
const ProjectTypeSelector = ({selected_value, handleChange, clearable = true, disabled = false, important = false, props} : Props) => {
    ...
    if(isActive) {
      ...
      if(selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Dropdowns/GetProjectsTypes`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if(isActive) {
      if((store.projects_types?.length || store.projects_types_success)) {
        setObjectList(store.projects_types)
      } else {
        dispatch(
          getProjectTypes({})
        )
  ...
  return (
    <CustomAutocomplete
      options={objectList??[]}
      sx={{ mb: 3 }}
      id='autocomplete-custom-project-type'
      onChange={onChange}
      value={object}
      disableClearable={!clearable}
      disabled={disabled??false}
      getOptionLabel={(option: GenericDropdownObj) => option?.text || ''}
      renderInput={params => <CustomTextField {...params} fullWidth label={<TypoLabel name={t('Project Type')} important={important??false} />} placeholder={t("Project Type") as string} {...(props??{})} />}
    />
  )
}

export default React.memo(ProjectTypeSelector);
