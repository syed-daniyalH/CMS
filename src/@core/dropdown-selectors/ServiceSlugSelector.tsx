...
import { getServiceLinksDropdown } from 'src/store/dropdowns/cms'
...
const ServiceSlugSelector = ({ selectedValue, handleChange, disabled = false, important = false }: Props) => {
  const dispatch = useDispatch<AppDispatch>()
  const links = useSelector((state: RootState) => state.cmsDropdowns?.serviceLinks ?? [])
  const [value, setValue] = useState<any | null>(null)

  useEffect(() => {
    if (!links.length) dispatch(getServiceLinksDropdown())
  ...
  return (
    <CustomAutocomplete
      ...
      renderInput={params => <CustomTextField {...params} fullWidth placeholder='Select service' />}
    />
  )
}

export default ServiceSlugSelector
