...
const MenuItemSubtitle = styled(Typography)<TypographyProps>({
  ...
  WebkitLineClamp: 2, // Max number of lines
  textOverflow: 'ellipsis'
})
...
const NotificationDropdown = (props: Props) => {
  ...
  const RenderAvatar = ({ notification }: { notification: any }) => {
    const { senderImage, senderUserName } = notification

    if (senderImage) {
      return <Avatar alt={senderUserName??""} src={senderImage} />
    } else {
      return (
        <Avatar skin='light' color={'info'}>
          {getInitials(senderUserName as string)}
        </Avatar>
      )
    }
  }
  ...
  return (
    <Fragment>
      <IconButton color='inherit' aria-haspopup='true' onClick={handleDropdownOpen} aria-controls='customized-menu'>
        ...
        >
          <Icon  color={'#ffffff'} fontSize='1.525rem' icon='tabler:bell' />
        </Badge>
      </IconButton>
      <Menu
        anchorEl={anchorEl}
            ...
            <MenuItem key={index} disableRipple disableTouchRipple onClick={handleDropdownClose}>
              <Box sx={{ width: '100%', display: 'flex', alignItems: 'center' }}>
                ...
                <Box sx={{ mr: 4, ml: 2.5, flex: '1 1', display: 'flex', overflow: 'hidden', flexDirection: 'column' }}>
                  <MenuItemTitle variant='subtitle2'>{notification.title}</MenuItemTitle>
                  <MenuItemSubtitle variant='caption'>{notification.description}</MenuItemSubtitle>
                </Box>
