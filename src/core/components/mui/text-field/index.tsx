...
const CustomTextField = forwardRef((props: CustomTextFieldProps, ref) => {
  // ** Props
  const { size = 'small', InputLabelProps, noBorder, ...rest } = props
  ...
  return (
    <TextFieldStyled
      ...
      {...rest}
      onChange={(e) => {
        if(rest.type === 'number' && !rest.error) {
          if(parseFloat(e.target.value) > 99999999999 || parseFloat(e.target.value) < 0) {
            setValue("Please Enter a valid value.");
            return;
          } else {
            setValue(null);
          }
      ...
      //@ts-ignore
      sx={noBorder ? {...props.sx, '& .MuiInputBase-root': {
        border: '0px solid',
          '&.Mui-focused': {
