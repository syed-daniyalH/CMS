...
import { getCaseStudyCategoriesDropdown } from 'src/store/dropdowns/cms'
...
const CaseStudyCategorySelector = ({ selectedValue, handleChange, disabled = false, important = false }: Props) => {
  const dispatch = useDispatch<AppDispatch>()
  const categories = useSelector((state: RootState) => state.cmsDropdowns?.caseStudyCategories ?? [])
  const [value, setValue] = useState<any | null>(null)

  useEffect(() => {
    if (!categories.length) dispatch(getCaseStudyCategoriesDropdown())
...
export default CaseStudyCategorySelector
