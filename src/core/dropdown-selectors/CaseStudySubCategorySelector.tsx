...
import { getCaseStudySubCategoriesDropdown } from 'src/store/dropdowns/cms'
...
const CaseStudySubCategorySelector = ({ selectedValue, parentCategoryId, handleChange, disabled = false, important = false }: Props) => {
  const dispatch = useDispatch<AppDispatch>()
  const subCategories = useSelector((state: RootState) => state.cmsDropdowns?.caseStudySubCategories ?? [])
  const params = useSelector((state: RootState) => state.cmsDropdowns?.caseStudySubCategoriesParams ?? {})
  const [value, setValue] = useState<any | null>(null)

  useEffect(() => {
    if (!subCategories.length || `${params?.parentCategory ?? ''}` !== `${parentCategoryId ?? ''}`) {
      dispatch(getCaseStudySubCategoriesDropdown({ parentCategory: parentCategoryId || undefined }))
    }
...
export default CaseStudySubCategorySelector
