...
import {GenericDropdownObj, getEmployeeContracts} from "../../store/dropdowns";
...
const EmployeeContractSelector = ({selected_value, handleChange, clearable = true, readOnly = false, disabled = false, props}: Props) => {
    ...
    if (isActive) {
      ...
      if (selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Defaults/GetPayRollEmployeeContractTypes`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if (isActive) {
      ...
      if (store.contract_types?.length || store.contract_types_success) {
        setObjectList(store.contract_types)
      } else {
        dispatch(
...
export default React.memo(EmployeeContractSelector);
