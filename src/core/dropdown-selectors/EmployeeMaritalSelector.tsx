...
import {GenericDropdownObj, getEmployeeMarital} from "../../store/dropdowns";
...
const EmployeeMaritalSelector = ({selected_value, handleChange, clearable = true, readOnly = false, disabled = false, props}: Props) => {
  ...
  useEffect(() => {
    ...
    if (isActive) {
      ...
      if (store.marital_status?.length || store.marital_status_success) {
        setObjectList(store.marital_status)
      } else {
        dispatch(
          getEmployeeMarital({})
        )
...
export default React.memo(EmployeeMaritalSelector);
