...
import { GenericDropdownObj, getDefaultFloors } from '../../store/dropdowns'
...
const FloorSelector = ({selected_value, handleChange, clearable = true, disabled = false, important = false, props} : Props) => {
    ...
    if(isActive) {
      ...
      if(selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Dropdowns/GetActiveFloors`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if(isActive) {
      if((store.default_floors?.length || store.default_floors_success)) {
        setObjectList(store.default_floors)
      } else {
        dispatch(
          getDefaultFloors({})
        )
  ...
  return (
    <CustomAutocomplete
      ...
      renderInput={params => <CustomTextField {...params} fullWidth label={<TypoLabel name={t(`${getProjectDetail(user?.projectType??1)}`)} important={important??false} />} placeholder={t(`${getProjectDetail(user?.projectType??1)}`) as string} {...(props??{})} />}
    />
  )
}
