...
import {GenericDropdownObj, getSalesStatus} from "../../store/dropdowns";
...
const InvoiceStatusSelector = ({selected_value, handleChange, clearable = true, disabled = false, important = false, props} : Props) => {
    ...
    if(isActive) {
      ...
      if(selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Defaults/SalesStatuses`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if(isActive) {
      ...
      if(store.sales_status?.length) {
        setObjectList(store.sales_status)
      } else {
        dispatch(
          getSalesStatus({})
        )
  ...
  return (
    <CustomAutocomplete
      ...
      renderInput={params => <CustomTextField {...params} fullWidth label={<TypoLabel name={t('Status')} important={important??false} />} {...(props??{})} />}
    />
  )
}

export default React.memo(InvoiceStatusSelector);
