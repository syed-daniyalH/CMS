...
import { GenericDropdownObj, getPreferences } from '../../store/dropdowns'
...
const PreferenceSelector = ({selected_value, handleChange, clearable = true, disabled = false, important = false, props} : Props) => {
    ...
    if(isActive) {
      ...
      if(selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Dropdowns/GetActivePrefrence`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if(isActive) {
      if((store.preferences?.length || store.preferences_success)) {
  ...
  return (
    <CustomAutocomplete
      ...
      renderInput={params => <CustomTextField {...params} fullWidth label={<TypoLabel name={t('Preferences')} important={important??false} />} placeholder={t("Preferences") as string} {...(props??{})} />}
    />
  )
}
