...
import { GenericDropdownObj, getPropertyListSelector } from '../../store/dropdowns'
...
const PropertyListSelector = ({selected_value, handleChange, clearable = true, disabled = false, important = false, preview = false, props} : Props) => {
    ...
    if(isActive) {
      ...
      if(selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Dropdowns/GetAllPropertyList`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if(isActive) {
      if((store.property_list?.length || store.property_list_success)) {
        setObjectList(store.property_list)
      } else {
        dispatch(
          getPropertyListSelector({})
        )
      }
    }
  ...
  return preview ? (
    ...
    <CustomAutocomplete
      ...
      id='autocomplete-custom-prop-type'
      onChange={onChange}
      value={object}
      disableClearable={!clearable}
      disabled={disabled??false}
      getOptionLabel={(option: GenericDropdownObj) => option?.text || ''}
      renderInput={params => <CustomTextField {...params} fullWidth label={<TypoLabel name={t('Property')} important={important??false} />} placeholder={t("Property") as string} {...(props??{})} />}
    />
  )
}

export default React.memo(PropertyListSelector);
