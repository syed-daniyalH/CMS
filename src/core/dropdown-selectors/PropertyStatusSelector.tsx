...
import { GenericDropdownObj, getPropertyStatus } from '../../store/dropdowns'
...
const PropertyStatusSelector = ({selected_value, handleChange, clearable = true, disabled = false, important = false, preview = false, props} : Props) => {
    ...
    if(isActive) {
      ...
      if(selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Dropdowns/GetActivePropStatus`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if(isActive) {
      if((store.property_status?.length || store.property_status_success)) {
        setObjectList(store.property_status)
      } else {
        dispatch(
          getPropertyStatus({})
        )
  ...
  }, [store.property_status, dispatch])
  ...
  return preview ? (
    ...
    <CustomAutocomplete
      options={objectList??[]}
      sx={{ mb: 3 }}
      id='autocomplete-custom-prop-status'
      onChange={onChange}
      value={object}
      disableClearable={!clearable}
      disabled={disabled??false}
      getOptionLabel={(option: GenericDropdownObj) => option?.text || ''}
      renderInput={params => <CustomTextField {...params} fullWidth label={<TypoLabel name={t('Property Status')} important={important??false} />} placeholder={t("Property Status") as string} {...(props??{})} />}
    />
  )
}

export default React.memo(PropertyStatusSelector);
