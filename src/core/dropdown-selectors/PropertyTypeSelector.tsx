...
import { GenericDropdownObj, getPropertyType } from '../../store/dropdowns'
...
const PropertyTypeSelector = ({selected_value, handleChange, clearable = true, disabled = false, important = false, preview = false, props} : Props) => {
    ...
    if(isActive) {
      ...
      if(selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Dropdowns/GetActivePropTypes`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  useEffect(() => {
    ...
    if(isActive) {
      if((store.property_types?.length || store.property_types_success)) {
        setObjectList(store.property_types)
      } else {
        dispatch(
          getPropertyType({})
        )
  ...
  return preview ? (
    ...
    <CustomAutocomplete
      ...
      renderInput={params => <CustomTextField {...params} fullWidth label={<TypoLabel name={t('Property Type')} important={important??false} />} placeholder={t("Property Type") as string} {...(props??{})} />}
    />
  )
}

export default React.memo(PropertyTypeSelector);
