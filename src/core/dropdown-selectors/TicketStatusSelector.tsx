...
import {GenericDropdownObj, getTicketStatus} from "../../store/dropdowns";
...
const TicketStatusSelector = ({selected_value, handleChange, clearable = true, disabled = false, props}: Props) => {
  ...
  useEffect(() => {
    ...
    if (isActive) {
      ...
      if (store.ticket_status?.length || store.ticket_status_success) {
        setObjectList(store.ticket_status)
      } else {
        dispatch(
          getTicketStatus({})
        )
  ...
  return (
    <>
      <CustomAutocomplete
        ...
        renderInput={params => <CustomTextField {...params} fullWidth label={t(statusKey)} placeholder={t(statusKey) as string} {...(props ?? {})} />}
...
export default React.memo(TicketStatusSelector);
