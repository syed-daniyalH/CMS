...
import {GenericDropdownObj, getUsers} from "../../store/dropdowns";
...
const UsersSelector = ({selected_value, handleChange, clearable = true, disabled = false, props} : Props) => {
    ...
    if(isActive) {
      ...
      if(selected_value && object?.value !== selected_value) {
        if (objectList.length) {
          ...
          axios.get(`/Defaults/LocationUsers`, {params: {Recno: selected_value}}).then(response => response.data).then((data: AxiosResponse<any>) => {
  ...
  return (
    <CustomAutocomplete
      ...
      renderInput={params => <CustomTextField {...params} fullWidth label={t('User')} {...(props??{})} />}
    />
  )
}
