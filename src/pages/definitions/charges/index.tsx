...
// ** Components Imports
import TableCharges from "src/views/definitions/charges/list/TableCharges";
import Icon from 'src/@core/components/icon';
import CustomBackdrop from "src/@core/components/loading";
import {useSelector} from "react-redux";
import {RootState} from "src/store";
import {useState} from "react";
import ChargesForm, {ChargesSchema} from "src/views/definitions/charges/form";
...
const ChargesList = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<ChargesSchema | null>(null)


  const toggleForm = (data?: ChargesSchema | null) => {
    if(data) {
      setSelectedRow(data);
    } else {
      setSelectedRow(null)
  ...
  return (
    <Card sx={{display: 'flex', flexDirection: 'column', height: '100%', borderRadius: 0}}>
      ...
      <TableCharges toggleForm={toggleForm}/>

      <CustomBackdrop open={(store.loadingState.getData??false)} />

      {
        openForm &&
          <ChargesForm open={openForm} toggle={toggleForm} data={selectedRow} />
      }
...
ChargesList.contentHeightFixed = true;
