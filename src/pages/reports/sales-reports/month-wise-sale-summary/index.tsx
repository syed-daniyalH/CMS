...
// ** Custom imports
import ActionBar from "src/views/reports/sales-reports/month-wise-sale-summary/ActionBar";
import FilterDrawer from "src/views/reports/sales-reports/month-wise-sale-summary/filter";
import MonthWiseSaleSummaryReport from "src/views/reports/sales-reports/month-wise-sale-summary/MonthWiseSaleSummaryReport";
...
// ** Types
import {SaleSummaryMonthWise, } from "src/graph-ql/sale-reports/MonthWiseSaleSummary";
...
//** decode Filter
        ...
        setPaginationModel({
          ...
          last: null,
            ...
            if (paginationModel.page > model.page) {
                ...
                setPaginationModel({
                    ...model,
                    before: dataWithPagination?.saleSummaryMonthWise?.pageInfo?.startCursor,
                    after: null,
                ...
                setPaginationModel({
                    ...model,
                    after: dataWithPagination?.saleSummaryMonthWise?.pageInfo?.endCursor,
                    before: null,
    ...
    const fetchData = async (exportType: string, fromPdf = false) => {
        try {
            ...
            const rows =  data?.saleSummaryMonthWise?.nodes ?? [];
                ...
                <Grid container>
                    <Grid item lg={12} md={12} sm={12} xs={12}>
                        <ActionBar
                          ...
                          data={dataWithPagination?.saleSummaryMonthWise?.nodes ?? []} queries={queries} loadingPdfPrint={loadingPdfPrint}  loadingExcel={loadingExcel} state={state} handlePrint={handlePrint} handleBackClick={handleBackClick} loadingPdf={loadingPdf} setState={setState}
                    ...
                    {loadingData || !queries?.fromDate? (
                        ...
                        <Grid item lg={12} md={12} sm={12} xs={12}>
                            <MonthWiseSaleSummaryReport queries={queries}
                                               selected_columns={columns.sort((a: any, b: any) => a.sortNo - b.sortNo)}
                                               state={state} data={dataWithPagination?.saleSummaryMonthWise?.nodes ?? []}
                                               totalRows={dataWithPagination?.saleSummaryMonthWise?.totalCount ?? 0}
