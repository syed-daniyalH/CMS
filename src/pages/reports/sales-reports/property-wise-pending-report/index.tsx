...
// ** Custom imports
import ActionBar from "src/views/reports/sales-reports/property-wise-pending-report/ActionBar";
import FilterDrawer from "src/views/reports/sales-reports/property-wise-pending-report/filter";
import PropertyWisePending from "src/views/reports/sales-reports/property-wise-pending-report/PropertyWisePending";
...
// ** Types
import {GET_PROPERTY_WISE_REPORT, } from "src/graph-ql/sale-reports/property-wise-pending";
...
const PropertyWisePendingReport = () => {
    const {user} = useAuth();
  ...
  const [columns, setColumns] = useState<any>([
    // 🕓 Sale Information
    { columnName: 'customerName', columnTitle: t("Customer Name/Date"), isVisible: true, sortNo: 2, isAlignRight: false, clickable: false, formatedName: null,width: 250, },
  { columnName: 'plotAmount', columnTitle: t("Total Price"), isVisible: true, sortNo: 7, isAlignRight: true, clickable: false, formatedName: null,width: 250, },
  { columnName: 'totalInRecAmount', columnTitle: t("Paid"), isVisible: true, sortNo: 8, isAlignRight: true, clickable: false, formatedName: null,width: 250, },
  { columnName: 'totalPendingAmount', columnTitle: t("Balance"), isVisible: true, sortNo: 9, isAlignRight: true, clickable: false, formatedName: null,width: 250, },
  { columnName: 'totalInPendingAmount', columnTitle: t("Due Payment Till Date"), isVisible: true, sortNo: 9, isAlignRight: true, clickable: false, formatedName: null,width: 250, },
    // 📐 Property Size & Rate
...
//** decode Filter
          ...
          let totalDuePayment = rows?.reduce((acc:number,curr:any)=>acc+(curr?.totalInPendingAmount)??0,0)
            ...
            setLoadingPdf(false)
              return flattened
                ...
                exportToExcel("",flattened, filteredColumns, t("Property Wise Pending Report"), (`${user?.tenantDetails[((user?.tenantDetails ?? []).findIndex(e => e.tenantId === user?.loginTenantId))]?.organizationName ?? "No Name"}`), `${formatDate(new Date(queries?.fromDate ?? '2023-12-01'))} - ${formatDate(new Date(queries?.toDate ?? '2023-12-01'))}`);
    ...
    return (
            ...
            <DatePickerWrapper>
                <Grid container>
                    ...
                    {loadingData || !queries?.fromDate? (
                        ...
                        <Grid item lg={12} md={12} sm={12} xs={12}>
                            <PropertyWisePending queries={queries}
      ...
      PropertyWisePendingReport.contentHeightFixed = true;
      PropertyWisePendingReport.acl = {
        action:"read",
        subject: 'SaleAgreement'
      }

export default PropertyWisePendingReport;
