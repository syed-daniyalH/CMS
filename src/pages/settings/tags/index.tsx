...
import TagTable from 'src/views/settings/tags/TagTable'
import TagFormDrawer, { TagSchema } from 'src/views/settings/tags/TagFormDrawer'
...
const TagList = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<TagSchema | null>(null)

  const toggleForm = (row?: TagSchema | null) => {
  ...
  return (
    <Card sx={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: 0 }}>
      ...
      <Divider />
      <TagTable toggleForm={toggleForm} />
      <CustomBackdrop open={store?.loadingState?.getData ?? false} />
      {openForm && <TagFormDrawer open={openForm} toggle={toggleForm} data={selectedRow} />}
    </Card>
  )
}

TagList.contentHeightFixed = true
