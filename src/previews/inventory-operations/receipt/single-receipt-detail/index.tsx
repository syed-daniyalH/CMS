...
import {singleReceiptDataType} from "./preview/type";

import { GET_SINGLE_RECEIPT_DETAIL } from 'src/graph-ql/receipts/SingleReceiptDetail'
import { useQuery } from '@apollo/client'

interface Props {
  recepitId: {recno:number}
  open: boolean
  toggle: (data: any) => void
}

const SingleReceiptDetail = ({open, toggle, recepitId}: Props) => {
  ...
  const {
    ...
    loading ,
    error
  } = useQuery(GET_SINGLE_RECEIPT_DETAIL, {
    variables: { recepitId: Number(recepitId?.recno) },
    skip: !recepitId,
            ...
            <Grid container spacing={1}>
              <Grid item xs={12} sm={12} md={12} lg={12}>
                <Box sx={{height: `calc(100vh - 60px)`, overflow: 'hidden', width: '100%'}}>
                  {
                    <DocumentPreview loading={loading} item={data?.singleRecepitData?.nodes[0]}/>
                  }
                </Box>
              </Grid>
            </Grid>
...
export default SingleReceiptDetail
