...
import { GET_INSTALLMENT_PLAN } from 'src/graph-ql/sale-agreement/InstallmentPlan'
...
const InstallmentPlanPreview = ({open, toggle, propertyId}: Props) => {
  ...
  const {
    ...
    loading ,
    error
  } = useQuery(GET_INSTALLMENT_PLAN, {
    variables: {
        propertyId: +propertyId?.recno

    },
    skip: !propertyId?.recno,
  ...
  return (
    <Drawer
      ...
      sx={{'& .MuiDrawer-paper': {width: '70%', backgroundColor: '#00000000', my: '10px', height: '98vh', borderRadius: '12px'}}}
            ...
            <Grid container spacing={1}>
              <Grid item xs={12} sm={12} md={12} lg={12}>
                <Box sx={{height: `calc(100vh - 60px)`, overflow: 'hidden', width: '100%'}}>
                  {
                    <DocumentPreview loading={loading} item={data?.singlePropertyData?.nodes[0]}/>
                  }
                </Box>
              </Grid>
            </Grid>
