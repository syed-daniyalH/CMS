...
export interface BlogCategorySearchParams {
  PageNo: number
  PageSize: number
  search?: string
}
...
type CategoryState = {
  data: any[]
  totalRecords: number
  params: BlogCategorySearchParams
  loadingState: LoadingStateParams
}

export const setLoading = createAsyncThunk('appBlogCategory/setLoading', async (loadingState: LoadingStateParams) => ({ ...loadingState }) as any)

export const getData = createAsyncThunk('appBlogCategory/getData', async (params: BlogCategorySearchParams, { dispatch }) => {
  dispatch(setLoading({ getData: true }))
  const response = await axiosInstance.get('/api/taxonomy/blog-categories')
...
const initialParams: BlogCategorySearchParams = {
  PageNo: 1,
  PageSize: 10,
  search: ''
}

export const appBlogCategory = createSlice({
...
export default appBlogCategory.reducer
