...
import { getData } from 'src/store/blogs/blog'
import {
  getBlogCategoriesDropdown,
  getBlogSubCategoriesDropdown,
  getBlogTagsDropdown,
...
import BlogSubCategorySelector from 'src/@core/dropdown-selectors/BlogSubCategorySelector'
...
const BlogForm = () => {
  ...
  const [state, setState] = useState<BlogState>(defaultState)
  ...
  const tags = useSelector((store: RootState) => store.cmsDropdowns?.blogTags ?? [])
  const customStyleMap = useMemo(() => {
    ...
    styles.forEach(style => {
      const color =
        extractStyleColor(style, ['COLOR-', 'color-']) ||
        extractStyleColor(style, ['COLOR', 'color'])
  ...
  useEffect(() => {
    dispatch(getBlogCategoriesDropdown())
    dispatch(getBlogSubCategoriesDropdown({ parentCategory: state.categoryId || undefined }))
    dispatch(getBlogTagsDropdown())
  ...
  const onSubmit = async (event: any, option: number) => {
    ...
    try {
      ...
      if (state.subCategoryId) payload.subCategoryId = state.subCategoryId
      ...
      if (isEdit && state._id) {
        await axiosInstance.put(`/api/blogs/${state._id}`, payload)
  ...
  const uploadEditorImage = async (file: File) => {
    const submitUpload = async (fieldName: 'objfile' | 'file') => {
      ...
      formData.append('altText', state.title || file.name || 'blog-content-image')
      formData.append('folder', 'images/blogs')
      formData.append('type', 'blog')
      const entityBase = state._id || blogId || state.slug || 'blog'
        ...
        <Box sx={{ mx: 'auto' }}>
          <Card>
            <Box sx={{ px: 4, py: 2.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Box>
                <Typography variant='h5'>{isEdit ? 'Update Blog' : 'Add Blog'}</Typography>
            ...
            <Box sx={{ p: 4 }}>
              ...
              <Grid container spacing={3}>
                <Grid item xs={12} md={6}>
                  <GlobalImageUploader
                    ...
                    folder='images/blogs'
                    type='blog'
                    entityId={state._id || blogId || state.slug}
                    uploadOnSubmit
                    multiple={false}
                    altText={state.title || 'blog-featured-image'}
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={4}>
                  <CustomTextField
                    select
                    fullWidth
                    label={<TypoLabel name='Blog Position Layout' important />}
                    value={state.layout}
                    onChange={e =>
                      setState(prev => ({
                        ...prev,
                        layout: (e.target.value as BlogState['layout']) ?? 'image-top'
                      }))
                    }
                  >
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={4}>
                  <BlogSubCategorySelector
                    ...
                    important
                    handleChange={value => setState(prev => ({ ...prev, subCategoryId: value?.id ?? value?._id ?? '' }))}
                  />
                </Grid>
                <Grid item xs={12} md={4}>
                  <CustomTextField
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={4}>
                  <CustomTextField fullWidth label={<TypoLabel name='SEO Keywords (comma separated)' />} value={state.seoKeywordsText} onChange={e => setState(prev => ({ ...prev, seoKeywordsText: e.target.value ?? '' }))} placeholder='blog, techionik, cms' />
                </Grid>
                <Grid item xs={12}>
