...
const BlogTable = () => {
  ...
  const resolveCategoryId = (value: any) => `${value?._id ?? value?.id ?? value?.categoryId ?? value?.value ?? ''}`.trim()
  ...
  }, [dispatch, paginationModel, searchValue, categoryId])
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      <Box sx={{ p: 2, display: 'flex', justifyContent: 'end', gap: 2 }}>
        <Box sx={{ width: 280 }}>
          <BlogCategorySelector
            ...
            handleChange={value => {
              setCategoryId(resolveCategoryId(value))
      ...
      <DataGrid
        ...
        columns={columns((id: string, slug: string) => router.push(`/blogs/add-blog?id=${id}&slug=${encodeURIComponent(slug || '')}`))}
