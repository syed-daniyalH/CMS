...
import { getData } from 'src/store/blogs/category'
import { saveCloseKey, saveNewKey } from 'src/@core/utils/translation-file'

export interface BlogCategorySchema {
  id?: string
  name?: string
  slug?: string
...
interface Props {
  open: boolean
  toggle: (data?: BlogCategorySchema | null) => void
  data?: BlogCategorySchema | null
}
...
const defaultState: BlogCategorySchema = {
  name: '',
  slug: '',
  shortDescription: '',
  isActive: true
...
const CategoryFormDrawer = ({ open, toggle, data }: Props) => {
  ...
  const [state, setState] = useState<BlogCategorySchema>(defaultState)
    ...
    try {
      ...
      if (state.id) {
        await axiosInstance.put(
          `/api/taxonomy/blog-categories/${state.id}`,
          payload
        )
      } else {
        await axiosInstance.post(
          '/api/taxonomy/blog-categories',
          payload
        )
      }

      toast.success(
        `Blog category ${state.id ? 'updated' : 'created'} successfully.`
      )
