...
import { CaseStudyCategorySearchParams, getData } from 'src/store/case-study/category'
import CustomChip from 'src/@core/components/mui/chip'
import type { CaseStudyCategorySchema } from './CategoryFormDrawer'

const columns = (toggleForm: (row: CaseStudyCategorySchema) => void): GridColDef[] => [
  {
    flex: 0.2,
    minWidth: 180,
    field: 'name',
...
const CategoryTable = ({ toggleForm }: { toggleForm: (row?: CaseStudyCategorySchema | null) => void }) => {
  ...
  useEffect(() => {
    const params: CaseStudyCategorySearchParams = {
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      ...
      <DataGrid
        ...
        columns={columns((row: CaseStudyCategorySchema) => toggleForm(row))}
