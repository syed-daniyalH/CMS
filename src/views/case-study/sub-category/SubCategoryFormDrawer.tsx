...
import { getData } from 'src/store/case-study/sub-category'
...
export interface CaseStudySubCategorySchema {
  id?: string
  name?: string
  slug?: string
...
interface Props {
  open: boolean
  toggle: (data?: CaseStudySubCategorySchema | null) => void
  data?: CaseStudySubCategorySchema | null
}
...
const defaultState: CaseStudySubCategorySchema = {
  name: '',
  slug: '',
  shortDescription: '',
  categoryId: '',
...
const SubCategoryFormDrawer = ({ open, toggle, data }: Props) => {
  ...
  const [state, setState] = useState<CaseStudySubCategorySchema>(defaultState)
  ...
  const onSubmit = async (event: any, index: number) => {
    ...
    if (!state.name?.trim()) return toast.error('Please enter sub category name.')
    if (!state.categoryId) return toast.error('Please select category.')
    ...
    try {
      ...
      if (state.id) await axiosInstance.put(`/api/taxonomy/case-study-sub-categories/${state.id}`, payload)
      else await axiosInstance.post('/api/taxonomy/case-study-sub-categories', payload)

      toast.success(`Case study sub category ${state.id ? 'updated' : 'created'} successfully.`)
  ...
  return (
    <Drawer
      ...
      sx={{ '& .MuiDrawer-paper': { alignItems: 'center', backgroundColor: '#00000000', marginTop: 2 } }}
      ModalProps={{ keepMounted: true }}
    >
      <Card sx={{ width: hidden ? '35%' : '98%', position: 'relative' }}>
        <Header>
          <Typography variant='h5'>{state?.id ? t('Update Case Study Sub Category') : t('Add Case Study Sub Category')}</Typography>
        ...
        <Box sx={{ p: 4, pb: 18, maxHeight: '85vh', overflowY: 'auto' }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <CustomTextField fullWidth label={<TypoLabel name='Name' important />} value={state.name ?? ''} onChange={e => setState(prev => ({ ...prev, name: e.target.value ?? '' }))} />
            </Grid>
            <Grid item xs={12}>
              <CustomTextField
                fullWidth
            ...
            <Grid item xs={12}>
              <CustomTextField fullWidth multiline rows={3} label={<TypoLabel name='Description' />} value={state.shortDescription ?? ''} onChange={e => setState(prev => ({ ...prev, shortDescription: e.target.value ?? '' }))} />
            </Grid>
            <Grid item xs={12}>
