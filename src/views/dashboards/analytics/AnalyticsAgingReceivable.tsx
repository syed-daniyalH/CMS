...
// ** Util Import
import {getReceivableAging} from "../../../store/dashboard";
...
const AnalyticsAgingReceivable = () => {
  // ** State
  ...
  useEffect(() => {
    ...
    if(isActive) {
      if(store.receivable_aging_data) {
        setBarSeries([{name: t("Received"), data: [store.receivable_aging_data?.currentAmount??0, store.receivable_aging_data?.last30Amount??0, store.receivable_aging_data?.last60Amount??0, store.receivable_aging_data?.above60Amount??0]}])
      }
    }
  ...
  return (
    <Card>
      <Grid container>
        ...
        >
          <CardHeader title={<Typography variant={'h4'}>{t('Aging-Receivable')}</Typography>} sx={{p: 3,backgroundColor: 'customColors.tableHeaderBg'}} />
        </Grid>
        <StyledGrid
          item
          sm={12}
...
export default AnalyticsAgingReceivable
