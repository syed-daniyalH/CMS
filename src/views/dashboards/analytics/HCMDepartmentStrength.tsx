...
const HCMDepartmentStrength = () => {
  ...
  useEffect(() => {
    ...
    if(isActive) {
      dispatch(getDepartmentStrength({}))
    }
  ...
  return (
    <Card sx={{ml: 2}}>
      <CardHeader title={t('Department Strength')} subheader={t("Department strength according to salary.")} sx={{px: 3, py: 2}}/>
      <Divider />
      <CardContent>
        {
                    ...
                    <TableRow key={index}>
                      <TableCell colSpan={3} style={{padding: '8px'}}>
                        <Box sx={{p: 0, display: 'flex', alignItems: 'center'}}>
                          <Typography variant={'subtitle2'} color='text.primary'>{item.departmentName}</Typography>
                        </Box>
                      </TableCell>
                      <TableCell colSpan={1} align={'left'} style={{padding: '8px'}}>
                        <Typography variant={'subtitle2'}>{item.totalEmployees}</Typography>
                      </TableCell>
                      <TableCell colSpan={1} align={'right'} style={{padding: '8px'}}>
                        <Typography variant={'subtitle2'} color='text.primary'>
                          {defaultCurrency?.code??defaultCurrencyCode} {kFormatter(item.totalSalary)}
                        </Typography>
                      </TableCell>
                    </TableRow>
                  ))}
...
export default HCMDepartmentStrength
