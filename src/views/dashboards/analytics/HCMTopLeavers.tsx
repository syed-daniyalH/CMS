...
import {getLastLeavers} from "../../../store/dashboard";
...
const HCMTopLeavers = () => {
  ...
  return (
    <Card sx={{ml: 2}}>
      <CardHeader title={t('Latest Leavers')} subheader={t("Latest leavers employees.")} sx={{px: 3, py: 2}}/>
      <Divider />
      <CardContent>
        {
                    ...
                    <TableRow key={index}>
                      ...
                      <TableCell colSpan={1} align={'right'} style={{padding: '8px'}}>
                        <Typography variant={'subtitle2'} color='text.primary'>
                          {dateFormat(new Date(item.leftDate??(new Date())), (user?.dateFormat??globalDateFormat).toLowerCase())}
                        </Typography>
                      </TableCell>
                    </TableRow>
                  ))}
