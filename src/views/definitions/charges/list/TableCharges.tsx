...
import {getData, ChargesSearchParams} from "src/store/definitions/charges";
import {decodeParameters, encodeParameters} from "src/@core/utils/encrypted-params";
import {useTranslation} from "react-i18next";
import ChargesTableToolbar from "./ChargesTableToolbar";
...
const columns= (toggleEdit: any, t: any) :GridColDef[] =>{
  return [
    ...
    },
    {
      flex: 0.175,
      minWidth: 120,
      headerName: t('Allow Duplicate'),
      field: 'isAllowDuplicate',
      ...
      renderCell: (params: GridRenderCellParams) => params.row && (
        <CustomChip
          ...
          size='small'
          skin='light'
          color={params.row?.isAllowDuplicate ? 'success' : "secondary"}
          label={params.row?.isAllowDuplicate ? "Allow" : "Not-Allowed"}
...
const TableCharges = ({toggleForm}: any) => {
  ...
  // ** States
  const [searchData, setSearchData] = useState<ChargesSearchParams>({PageNo: 1, PageSize: 50})
  ...
  return (
    <Box sx={{flexGrow: 1}}>
      ...
      <DataGrid
        ...
        slots={{toolbar: ChargesTableToolbar}}
