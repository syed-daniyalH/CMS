...
import {getData, DocumentTypeSearchSchema} from "src/store/definitions/document-type";
    ...
    },
    {
      ...
      renderCell: (params: GridRenderCellParams) => params.row && (
        <CustomChip
          ...
          size='small'
          skin='light'
          color={params.row?.isReq ? 'error' : "secondary"}
          label={params.row?.isReq ? "Required" : "Not-Required"}
...
const TableDocumentType = ({toggleForm}: any) => {
  ...
  // ** States
  const [searchData, setSearchData] = useState<DocumentTypeSearchSchema>({PageNo: 1, PageSize: 50})
  ...
  return (
    <Box sx={{flexGrow: 1}}>
      ...
      <DataGrid
        ...
        slots={{toolbar: DocumentTypeTableToolbar}}
