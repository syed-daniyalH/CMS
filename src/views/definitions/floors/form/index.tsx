...
interface Props {
  data?: FloorsSchema | null
  open: boolean
  toggle: (data?: FloorsSchema | null) => void
  onSuccess?: (data: any) => void
}

export interface FloorsSchema {
  groupId?: number | null
...
const FloorsForm = ({open, toggle, data, onSuccess}: Props) => {

  const {t} = useTranslation();
  const theme = useTheme()
  const hidden = useMediaQuery(theme.breakpoints.up('md'));
  const [loading, setLoading] = useState<boolean>(false);
  const [state, setState] = useState<FloorsSchema>({isActive: true});
  ...
  const getFloorsData = async () => {
    try {
      let res = await axios.get(`/Floors/${data?.groupId}`);
  ...
  const onSubmit = async (event: any, index: number) => {
    ...
    try {
      ...
      if(errorMessage.length > 1) {
        toast.error(`Error while creating floors!\n${errorMessage}`)
        return;
      }

      let response: any = state.groupId ? await axios.put('/Floors', state) : await axios.post('/Floors', state)
      ...
      if(res?.succeeded) {
        ...
        toast.error(`Error while floors!. ${message}`)
      }
    } catch (res: any) {
      let message : any = res.response?.message || res.response?.data?.message || res.data?.message || res.message || res.data;
      toast.error(`Error while creating floors!. ${message}`)
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <Card sx={{width: hidden ? '30%' : '98%'}}>
        <Header sx={{px: 4, py: 2}}>
          <Typography variant='h5'>{state?.groupId ? t(`Update ${getProjectDetail(user?.projectType??1)}`) : t(`Add ${getProjectDetail(user?.projectType??1)}`)}</Typography>
          <IconButton
            size='small'
        ...
        <Box sx={{p: theme => theme.spacing(4, 6, 16), position: 'relative', maxHeight: '90vh', overflow: 'auto'}}>
          ...
          <Box sx={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
            <CustomAvatar skin='light' color='secondary' variant='rounded' sx={{mr: 4, width: 114, height: 114}}>
              <Icon icon='tabler:building-arch' fontSize={'4.5rem'}/>
            </CustomAvatar>
          </Box>

          <Grid container spacing={2} sx={{mt: 3}}>
            <Grid item xs={12} sm={12} lg={12}>
              <FloorSelector selected_value={state.parentGroupId??null} handleChange={(value) => {
                setState({...state, parentGroupId: value?.value ? parseInt(`${value?.value}`) : null})
              }} />
            </Grid>
            <Grid item xs={12} sm={12} lg={12}>
              <CustomTextField
