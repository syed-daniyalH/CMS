...
import { getData } from 'src/store/definitions/installment-type';
...
// import { getDefaultPropStatus } from '../../../../store/dropdowns'


interface Props {
  data?: InstallmentTypeSchema | null
  open: boolean
  toggle: (data?: InstallmentTypeSchema | null) => void
  onSuccess?: (data: any) => void
}

export interface InstallmentTypeSchema {
...
const InstallmentTypeForm = ({open, toggle, data, onSuccess}: Props) => {

  const {t} = useTranslation();
  const theme = useTheme()
  const hidden = useMediaQuery(theme.breakpoints.up('md'));
  const [loading, setLoading] = useState<boolean>(false);
  const [state, setState] = useState<InstallmentTypeSchema>({isActive: true});
  ...
  const getPropStatusData = async () => {
    try {
      let res = await axios.get(`/InstTypes/${data?.instTypeId}`);
      setState({...(res?.data?.data)})
    } catch (e: any) {
      toast.error(t('Unable to load the installment data.'));
      toggle(null);
    }
  }
  ...
  const onSubmit = async (event: any, index: number) => {
    ...
    try {
      ...
      if(errorMessage.length > 1) {
        toast.error(`Error while creating installment types !\n${errorMessage}`)
        return;
      }

      let response: any = state.instTypeId ? await axios.put('/InstTypes', state) : await axios.post('/InstTypes', state)
      let res = response?.data;
      if(res?.succeeded) {
        toast.success("Installment Types  Saved Successfully.");
        ...
        toast.error(`Error while installment types !. ${message}`)
      }
    } catch (res: any) {
      let message : any = res.response?.message || res.response?.data?.message || res.data?.message || res.message || res.data;
      toast.error(`Error while creating installment types !. ${message}`)
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <Card sx={{width: hidden ? '30%' : '98%'}}>
        <Header sx={{px: 4, py: 2}}>
          <Typography variant='h5'>{state?.instTypeId ? t("Update Installment Type") : t("Add Installment Type")}</Typography>
          <IconButton
            size='small'
        ...
        <Box sx={{p: theme => theme.spacing(4, 6, 16), position: 'relative', maxHeight: '90vh', overflow: 'auto'}}>
          ...
          <Box sx={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
            <CustomAvatar skin='light' color='secondary' variant='rounded' sx={{mr: 4, width: 114, height: 114}}>
              <Icon icon='tabler:coins' fontSize={'4.5rem'}/>
            </CustomAvatar>
          </Box>
          ...
          <Grid container spacing={2} sx={{mt: 3}}>
            ...
            <Grid item xs={12} sm={12} lg={12}>
              <CustomTextField
                autoFocus
                fullWidth
                label={<TypoLabel name={'Month Gap'} important />}
                type={'number'}
                sx={{mb: 3}}
                variant='outlined'
                placeholder={t('Month Gap') as string}
                value={state?.monthGap??""}
                onChange={(event) => {
                  setState({...(state??{}), monthGap: +event.target.value??0})
                }}
              />
            </Grid>
...
export default InstallmentTypeForm
