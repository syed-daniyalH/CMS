...
import { getData } from 'src/store/definitions/preferences';
...
interface Props {
  data?: PreferencesSchema | null
  open: boolean
  toggle: (data?: PreferencesSchema | null) => void
...
const PreferencesForm = ({open, toggle, data, onSuccess}: Props) => {

  const {t} = useTranslation();
  const theme = useTheme()
  const hidden = useMediaQuery(theme.breakpoints.up('md'));
  const [loading, setLoading] = useState<boolean>(false);
  const [state, setState] = useState<PreferencesSchema>({isActive: true});
  ...
  useEffect(() => {
    ...
    if(isActive && data) {
      getPreferencesData().then(() => console.log('loaded'));
    }
  ...
  const getPreferencesData = async () => {
    try {
      let res = await axios.get(`/Prefrence/${data?.prefId}`);
      setState({...(res?.data?.data)})
    } catch (e: any) {
      toast.error(t('Unable to load the preferences data.'));
      toggle(null);
    }
  }
  ...
  const onSubmit = async (event: any, index: number) => {
    ...
    try {
      ...
      if(errorMessage.length > 1) {
        toast.error(`Error while creating preferences!\n${errorMessage}`)
        return;
      }

      let response: any = state.prefId ? await axios.put('/Prefrence', state) : await axios.post('/Prefrence', state)
      ...
      if(res?.succeeded) {
        ...
        toast.error(`Error while preferences!. ${message}`)
      }
    } catch (res: any) {
      let message : any = res.response?.message || res.response?.data?.message || res.data?.message || res.message || res.data;
      toast.error(`Error while creating preferences!. ${message}`)
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <Card sx={{width: hidden ? '30%' : '98%'}}>
        <Header sx={{px: 4, py: 2}}>
          <Typography variant='h5'>{state?.prefId ? t("Update Preference") : t("Add Preference")}</Typography>
          <IconButton
            size='small'
        ...
        <Box sx={{p: theme => theme.spacing(4, 6, 16), position: 'relative', maxHeight: '90vh', overflow: 'auto'}}>
          ...
          <Box sx={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
            <CustomAvatar skin='light' color='secondary' variant='rounded' sx={{mr: 4, width: 114, height: 114}}>
              <Icon icon='tabler:settings-2' fontSize={'4.5rem'}/>
            </CustomAvatar>
          </Box>
          ...
          <Grid container spacing={2} sx={{mt: 3}}>
            ...
            <Grid item xs={12} sm={12} lg={12}>
              <CustomTextField
                fullWidth
                label={<TypoLabel name={'Percentage'} important />}
                type={'number'}
                sx={{mb: 3}}
                variant='outlined'
                ...
                onChange={(event) => {
                  setState({...(state??{}), increasePercentage: +event.target.value??0})
                }}
              />
            </Grid>
            <Grid item xs={12} sm={12} lg={12}>
