...
import { getData } from 'src/store/definitions/prop-type';
...
import { getPropertyType } from '../../../../store/dropdowns'
import { MenuItem } from '@mui/material'


interface Props {
  data?: PropTypeSchema | null
  open: boolean
  toggle: (data?: PropTypeSchema | null) => void
  onSuccess?: (data: any) => void
}

export interface PropTypeSchema {
  typeId?: number | null
...
const PropTypeForm = ({open, toggle, data, onSuccess}: Props) => {

  const {t} = useTranslation();
  const theme = useTheme()
  const hidden = useMediaQuery(theme.breakpoints.up('md'));
  const [loading, setLoading] = useState<boolean>(false);
  const [state, setState] = useState<PropTypeSchema>({isActive: true});
  ...
  const getPropTypeData = async () => {
    try {
      let res = await axios.get(`/PropType/${data?.typeId}`);
  ...
  const onSubmit = async (event: any, index: number) => {
    ...
    try {
      ...
      if(errorMessage.length > 1) {
        toast.error(`Error while creating property type!\n${errorMessage}`)
        return;
      }

      let response: any = state.typeId ? await axios.put('/PropType', state) : await axios.post('/PropType', state)
      let res = response?.data;
      if(res?.succeeded) {
        toast.success("Property Type Saved Successfully.");
        ...
        toast.error(`Error while property type!. ${message}`)
      }
    } catch (res: any) {
      let message : any = res.response?.message || res.response?.data?.message || res.data?.message || res.message || res.data;
      toast.error(`Error while creating property type!. ${message}`)
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <Card sx={{width: hidden ? '30%' : '98%'}}>
        <Header sx={{px: 4, py: 2}}>
          <Typography variant='h5'>{state?.typeId ? t("Update Property Type") : t("Add Property Type")}</Typography>
          <IconButton
            size='small'
          ...
          <Grid container spacing={2} sx={{mt: 3}}>
            ...
            <Grid item xs={12} sm={12} lg={12}>
              <CustomTextField
                ...
                fullWidth
                multiline
                label={<TypoLabel name={'Remarks'} important />}
                type='text'
                sx={{mb: 3}}
                rows={3}
                variant='outlined'
