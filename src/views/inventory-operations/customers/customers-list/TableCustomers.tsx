...
const renderClient = (params: GridRenderCellParams) => {
  ...
  if ((row.customerImageUrl??"").length) {
    return <CustomAvatar src={`${process.env.NEXT_PUBLIC_IMAGE_URL}${row.customerImageUrl}`} sx={{ mr: 3, width: '1.875rem', height: '1.875rem' }} />
  } else {
    return (
      <CustomAvatar
        skin='light'
      ...
      return (
        ...
        <Card sx={{display: 'flex', alignItems: 'center'}}>
          <IconButton component={Link}
                      href={`/inventory-operations/customers/edit-customer/${encodeParameters({recno: params?.row?.customerId})}`}>
...
const TableCustomers = ({toggleFilter, openFilter}: Props) => {
  // ** States
  const [searchData, setSearchData] = useState<CustomerDataParams>({PageNo: 1, PageSize: 10})
  ...
  const handleCloseMenu = (option: number) => {
    switch (option) {
      case 1:
        navRouter.push(`/inventory-operations/customers/edit-customer/${encodeParameters({recno: hoveredRow?.recno})}`)
        break;
      case 2:
  ...
  return (
    <Box sx={{flexGrow: 1}}>
      <DataGrid
        ...
        apiRef={apiRef}
        slots={{ toolbar: selectedRows.length > 0 ? CustomersTableActions : null }}
