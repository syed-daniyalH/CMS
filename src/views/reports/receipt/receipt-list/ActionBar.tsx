...
import {ReceiptList} from "src/core/utils/form-types";
...
const ActionBar = ({data,state, handlePrint,queries, setState,handleBackClick, toggleFilter, updateQueries, fetchData, loadingPdf, loadingPdfPrint, loadingExcel,title,selected_columns}:any) => {
  ...
  const handlePrintClick = async (value: boolean) => {
    ...
    exportPDFReport({data, columns: selected_columns,title:"Receipt List", organizationName:title, download: value,   reportName:ReceiptList});
  };

  const handleCustomer = (updated: any) => {


    setState({...state,customerId:updated?.value,customerName:updated?.text })
  }
  return (
          ...
          smallScreen ? null :
            <Stack sx={{flexWrap: 'wrap'}}   direction={'row'} spacing={2}>
              <Box sx={{width: '200px', mb: 2 }}>
                <CustomerSelector selected_value={state?.customerId??null} handleChange={handleCustomer} props={{sx: {mb: 0}, label: null }} />                        </Box>
