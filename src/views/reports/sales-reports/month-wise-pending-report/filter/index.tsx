...
const FilterDrawer = ({ open, handleReset, toggle, state, setState ,queries, updateQueries}: Props) => {
  ...
  const {t} = useTranslation();
    ...
    return (
      ...
      >
        <Header>
          <Typography variant='h5'>{t("Pending Installments report Customization")}</Typography>
          <IconButton
            size='small'
            onClick={toggle}
            sx={{
        ...
        <Box sx={{p: theme => theme.spacing(0, 0, 6), height: 'calc(100vh - 130px)', overflow: 'hidden'}}>
                ...
                <TabContext value={value}>
                  ...
                  <TabPanel value='3' sx={{p: 0}}>
                    <Box sx={{height: 'calc(100vh - 130px)', overflow: 'auto'}}>
                      <ReportSettings reportName={MonthWisePending} reportTitle={"Month Wise Pending Report"} />
                    </Box>
                  </TabPanel>
                </TabContext>
          {
