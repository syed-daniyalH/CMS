...
const ActionBar = ({data,state, handlePrint,queries, setState,handleBackClick, toggleFilter, updateQueries, fetchData, loadingPdf, loadingPdfPrint, loadingExcel,title,selected_columns}:any) => {
  ...
  const handlePrintClick = async (value: boolean) => {
    ...
    exportPDFReport({data, columns: selected_columns,title:"Inventory Report",dateRange:{fromDate:queries?.fromDate,toDate:queries?.toDate}, organizationName:title, download: value,   reportName:InventoryList});
  };
                ...
                title={
                    ...
                    <Stack sx={{flexWrap: 'wrap'}}   direction={'row'} spacing={2}>
                      ...
                      <Box sx={{width: '200px', mb: 2}}>
                        <FloorSelector selected_value={state.floorId??null} handleChange={handleFloor} props={{sx: {mb: 0}, label: null }} />                      </Box>
