...
import PropertyListFilter from "./LedgerSummaryFilter";
...
import {InventoryList} from "../../../../../@core/utils/form-types";
...
const FilterDrawer = ({ open, handleReset, toggle, state, setState ,queries, updateQueries}: Props) => {
  ...
  const {t} = useTranslation();
    ...
    return (
      ...
      >
        <Header>
          <Typography variant='h5'>{t("Inventory Report Customization")}</Typography>
          <IconButton
            size='small'
            onClick={toggle}
            sx={{
                ...
                <TabContext value={value}>
                  ...
                  <TabPanel value='1' sx={{p: theme => theme.spacing(2, 3)}}>
                    <Box sx={{height: 'calc(100vh - 130px)', overflow: 'auto'}}>
                      ...
                      <PropertyListFilter queries={queries} state={state} setState={setState} open={open}/>

                        </Box>
                    </TabPanel>
                  <TabPanel value='3' sx={{p: 0}}>
                    <Box sx={{height: 'calc(100vh - 130px)', overflow: 'auto'}}>
                      <ReportSettings reportName={InventoryList} reportTitle={"Inventory Report"} />
                    </Box>
                  </TabPanel>
                </TabContext>
          {
