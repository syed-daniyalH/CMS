...
import {PropertyWisePending} from "src/@core/utils/form-types";
...
const ActionBar = ({data,state, handlePrint,queries, setState,handleBackClick, toggleFilter, updateQueries, fetchData, loadingPdf, loadingPdfPrint, loadingExcel,title,selected_columns}:any) => {
  ...
  const handlePrintClick = async (value: boolean) => {
    ...
    exportPDFReport({data, columns: selected_columns,title:"Property Wise Pending Report",dateRange:{fromDate:queries?.fromDate,toDate:queries?.toDate}, organizationName:title, download: value,   reportName:PropertyWisePending});
  };
