...
// ** Custom imports
import PropertyWisePendingTable from "./table/PropertyWisePendingTable";
...
const PropertyWisePending = ({clearAll,filters,state, queries, data, totalRows, paginationModel, setPaginationModel, selected_columns,setState,updateQueries,handleReset}: any) => {
  ...
  useEffect(()=>{
    ...
    return (
      <Card sx={{mt: '10px', height: `calc(100vh  - 135px)`, overflow: 'hidden'}}>
                ...
                title={
                    <Stack direction='column' spacing={2}>
                      ...
                      <Typography variant='body1' sx={{fontWeight: 600, fontSize:"18px", color:theme => theme.palette.customColors.textColor}}>
                            {t("Property Wise Pending Report")}
                        </Typography>
        ...
        <PropertyWisePendingTable filters={filters} queries={queries} selected_columns={selected_columns} state={state} data={data} totalRows={totalRows} paginationModel={paginationModel} setPaginationModel={setPaginationModel}   />
...
export default PropertyWisePending
