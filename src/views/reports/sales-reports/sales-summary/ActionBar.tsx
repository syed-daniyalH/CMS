...
import {SalesSummary} from "src/core/utils/form-types";
...
const ActionBar = ({data,state, handlePrint,queries, setState,handleBackClick, toggleFilter, updateQueries, fetchData, loadingPdf, loadingPdfPrint, loadingExcel,title,selected_columns}:any) => {
  ...
  const handlePrintClick = async (value: boolean) => {
    ...
    exportPDFReport({data, columns: selected_columns,title:"Sales Summary",dateRange:{fromDate:queries?.fromDate,toDate:queries?.toDate}, organizationName:title, download: value,   reportName:SalesSummary});
  };
