...
import {PropertyTransferDetail} from "src/core/utils/form-types";
import PropertyListSelector from '../../../../core/dropdown-selectors/PropertyListSelector'
import Grid from '@mui/material/Grid'
import { exportTransferPropertyPDF } from '../../sales-reports/property-wise-pending-report/pdf/AutoTablePdf'
...
const ActionBar = ({data,state, handlePrint,queries, setState,handleBackClick, toggleFilter, updateQueries, fetchData, loadingPdf, loadingPdfPrint, loadingExcel,title,selected_columns}:any) => {
  ...
  const handlePrintClick = async (value: boolean) => {
    ...
    exportTransferPropertyPDF({data, columns: selected_columns,title:"Property Transfer Detail",dateRange:{fromDate:queries?.fromDate,toDate:queries?.toDate}, organizationName:title, download: value,   reportName:PropertyTransferDetail});
  };
                ...
                title={
                    ...
                    <Stack sx={{flexWrap: 'wrap'}}   direction={'row'} spacing={2}>
                        <Box sx={{width: '200px', mb: 2 }}>
                          <PropertyListSelector selected_value={state.propertyTypeId??null} handleChange={handlePropertyList} props={{sx: {mb: 0},label:null  }} />
                        </Box>
                        {
                            state?.period === "custom" &&
                      ...
                      {/*<Box sx={{width: '200px', mb: 2}}>*/}
                      {/*  <DetailAccountSelector state={state} setState={setState} placeholder={t("Account")} size='small'/>*/}
                      {/*</Box>*/}
