...
const ServiceForm = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true)
      try {
        const res = await axiosInstance.get(`/api/menu-structure/services/${encodeURIComponent(slugParam)}`)
        ...
        toast.error(error?.response?.data?.message || error?.message || 'Failed to load service.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const save = async () => {
    ...
    if (!form.slug.trim()) return toast.error('Service slug is required.')
    setLoading(true)
    ...
    try {
      if (mode === 'edit') {
        await axiosInstance.put(`/api/menu-structure/services/${encodeURIComponent(slugParam)}`, payload)
        toast.success('Service updated successfully.')
      } else {
        await axiosInstance.post('/api/menu-structure/services', payload)
    ...
    <Box sx={{ flexGrow: 1, pb: { xs: 12, md: 16 } }}>
      <Card sx={{ borderRadius: 2, overflow: 'hidden', border: '1px solid', borderColor: 'divider' }}>
        <Box sx={{ p: { xs: 2, md: 3 }, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2 }}>
          <Box>
            <Typography variant='h5'>{mode === 'edit' ? 'Edit Service' : 'Add Service'}</Typography>
          </Box>
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Button variant='outlined' onClick={() => router.push('/services/list')}>
              Back
            </Button>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <GlobalImageUploader
                ...
                uploadOnSubmit
                multiple={false}
                altText={form.name || 'service-image'}
                value={form.imageUrl}
                onUploaded={items => {
            ...
            <Grid item xs={12} md={3}>
              <CustomTextField select fullWidth label='Scope' value='service' disabled>
                        ...
                        <Grid container spacing={2} sx={{ mt: 0.5 }}>
                          ...
                          <Grid item xs={12}>
                            <GlobalImageUploader
                              ...
                              folder='images/services'
                              type='service'
                              entityId={`${form.slug || form.name || 'service'}-hero-${index}`}
                              uploadOnSubmit
                              multiple={false}
                              altText={slide.imageAlt?.trim() || slide.title?.trim() || `service-hero-slide-${index + 1}`}
                              value={slide.image}
                              onUploaded={items => {
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                      <GlobalImageUploader
                        ...
                        uploadOnSubmit
                        multiple={false}
                        altText={form.template.ctaImageAlt || form.template.ctaTitle || 'service-cta-image'}
      ...
      <SaveActionBar onBack={() => router.push('/services/list')} onSave={save} saveLabel='Save & Close' loading={loading} />
