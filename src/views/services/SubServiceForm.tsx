...
const SubServiceForm = () => {
  ...
  const serviceSlugParam = `${router.query?.serviceSlug ?? ''}`.trim()
  ...
  useEffect(() => {
    const loadServices = async () => {
      try {
        ...
        setForm(prev => ({
          ...prev,
          parentServiceSlug: prev.parentServiceSlug || serviceSlugParam || options?.[0]?.slug || ''
        }))
      } catch (error: any) {
  ...
  useEffect(() => {
    if (mode !== 'edit' || !slugParam || !serviceSlugParam) return
    const load = async () => {
      setLoading(true)
      try {
        const res = await axiosInstance.get(
          `/api/menu-structure/services/${encodeURIComponent(serviceSlugParam)}/sub-services/${encodeURIComponent(slugParam)}`
        )
        const item = res?.data?.data ?? res?.data ?? {}
        setForm({
          parentServiceSlug: serviceSlugParam,
          name: `${item?.name ?? ''}`,
          ...
          industryIds: industryIdsFromPopulated(item?.industries),
          template: fromTemplateData(item?.templateData ?? {})
        })
        setSlugTouched(true)
      } catch (error: any) {
        toast.error(error?.response?.data?.message || error?.message || 'Failed to load sub service.')
      } finally {
        setLoading(false)
      }
    }
    load().then(() => null)
  }, [mode, slugParam, serviceSlugParam])

  const save = async () => {
    if (!form.parentServiceSlug.trim()) return toast.error('Parent service is required.')
    if (!form.name.trim()) return toast.error('Sub service name is required.')
    if (!form.slug.trim()) return toast.error('Sub service slug is required.')
    ...
    const nextTemplate = { ...form.template, heroSlides: nextHeroSlides, ctaImageUrl: uploadedCtaImageUrl.trim() }
    const payload = {
      name: form.name.trim(),
      slug: form.slug.trim(),
    ...
    try {
      if (mode === 'edit') {
        await axiosInstance.put(
          `/api/menu-structure/services/${encodeURIComponent(form.parentServiceSlug)}/sub-services/${encodeURIComponent(slugParam)}`,
          payload
        )
        toast.success('Sub service updated successfully.')
      } else {
        await axiosInstance.post(`/api/menu-structure/services/${encodeURIComponent(form.parentServiceSlug)}/sub-services`, payload)
        toast.success('Sub service created successfully.')
      }
      router.push(`/services/sub-services-list`)
    } catch (error: any) {
      toast.error(error?.response?.data?.message || error?.message || 'Failed to save sub service.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <Box sx={{ flexGrow: 1 }}>
      <Card sx={{ borderRadius: 2, overflow: 'hidden', border: '1px solid', borderColor: 'divider' }}>
        <Box sx={{ p: { xs: 2, md: 3 }, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2 }}>
          <Box>
            <Typography variant='h5'>{mode === 'edit' ? 'Edit Sub Service' : 'Add Sub Service'}</Typography>
          </Box>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              <CustomTextField
                ...
                value={form.parentServiceSlug}
                disabled={mode === 'edit'}
                onChange={e => setForm(prev => ({ ...prev, parentServiceSlug: e.target.value ?? '' }))}
              >
                {services.map(item => (
            ...
            <Grid item xs={12}>
              <GlobalImageUploader
                ...
                label='Sub Service Icon Image'
                folder='images/sub-services'
                type='sub-service'
                entityId={form.slug || form.name}
                uploadOnSubmit
                multiple={false}
                altText={form.name || 'sub-service-image'}
                value={form.imageUrl}
                onUploaded={items => {
            ...
            <Grid item xs={12}>
              <ServiceIndustriesMultiSelect
                label='Industry tags'
                value={form.industryIds}
                onChange={ids => setForm(prev => ({ ...prev, industryIds: ids }))}
              />
            </Grid>
            <Grid item xs={12}>
                          ...
                          <Grid container spacing={2} sx={{ mt: 0.5 }}>
                            ...
                            <Grid item xs={12}>
                              <GlobalImageUploader
                                ...
                                folder='images/sub-services'
                                type='sub-service'
                                entityId={`${form.slug || form.name || 'sub-service'}-hero-${index}`}
                                uploadOnSubmit
                                multiple={false}
                                altText={slide.imageAlt?.trim() || slide.title?.trim() || `sub-service-hero-slide-${index + 1}`}
                                value={slide.image}
                                onUploaded={items => {
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12}>
                        <GlobalImageUploader
                          ...
                          uploadOnSubmit
                          multiple={false}
                          altText={form.template.ctaImageAlt || form.template.ctaTitle || 'sub-service-cta-image'}
